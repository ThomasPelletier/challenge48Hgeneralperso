...
export const ServiceProduit = {
    getAllProduitByMarchand: async (marchand) => {
        let produits = await ServiceXhr.callWithoutAuthWithoutBody(`https://${marchand.instance.ip}:${process.env.VUE_APP_PORT_API}/api/produits/filtre/marchand/${marchand.id}`, "GET");
        let produitsFinaux = [];
